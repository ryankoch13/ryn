...
const PinInput: React.FC<PinInputProps> = ({ value, setValue, type }) => {
  const { spacing, fontSize, fontWeight, borderRadius } = theme
  const { active } = useTheme()
  const { height, width } = useWindowDimensions()
  const keypadContent = [1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, 'X']
  const keypadSize = width * 0.2
  const keypadTextSize = keypadSize * 0.4
  const pinLength = 4
  ...
  const styles = StyleSheet.create({
    container: {
      flex: 1,
      backgroundColor: active.grayscale.white,
